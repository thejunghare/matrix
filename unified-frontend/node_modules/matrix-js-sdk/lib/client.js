...
export class MatrixClient extends TypedEventEmitter {
  ...
  getVersions() {
    var _this47 = this;
    return _asyncToGenerator(function* () {
      if (_this47.serverVersionsPromise) {
        return _this47.serverVersionsPromise;
      }

      // We send an authenticated request as of MSC4026
      _this47.serverVersionsPromise = _this47.http.authedRequest(Method.Get, "/_matrix/client/versions", undefined, undefined, {
        prefix: ""
      }).catch(e => {
        // Need to unset this if it fails, otherwise we'll never retry
        _this47.serverVersionsPromise = undefined;
        // but rethrow the exception to anything that was waiting
        throw e;
      });
      var serverVersions = yield _this47.serverVersionsPromise;
      _this47.canSupport = yield buildFeatureSupportMap(serverVersions);
      return _this47.serverVersionsPromise;
    })();
  }
  ...
  doesServerSupportUnstableFeature(feature) {
    var _this49 = this;
    return _asyncToGenerator(function* () {
      var response = yield _this49.getVersions();
  ...
  doesServerSupportThread() {
    var _this51 = this;
    return _asyncToGenerator(function* () {
      if (yield _this51.isVersionSupported("v1.4")) {
        return {
      ...
      try {
        var [threadUnstable, threadStable, listUnstable, listStable, fwdPaginationUnstable, fwdPaginationStable] = yield Promise.all([_this51.doesServerSupportUnstableFeature("org.matrix.msc3440"), _this51.doesServerSupportUnstableFeature("org.matrix.msc3440.stable"), _this51.doesServerSupportUnstableFeature("org.matrix.msc3856"), _this51.doesServerSupportUnstableFeature("org.matrix.msc3856.stable"), _this51.doesServerSupportUnstableFeature("org.matrix.msc3715"), _this51.doesServerSupportUnstableFeature("org.matrix.msc3715.stable")]);
        return {
  ...
  relations(roomId, eventId, relationType, eventType) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var fetchedEventType = eventType ? _this52.getEncryptedIfNeededEventType(roomId, eventType) : null;
      var [eventResult, result] = yield Promise.all([_this52.fetchRoomEvent(roomId, eventId), _this52.fetchRelations(roomId, eventId, relationType, fetchedEventType, opts)]);
      ...
      if (fetchedEventType === EventType.RoomMessageEncrypted) {
        ...
        yield Promise.all(allEvents.map(e => _this52.decryptEventIfNeeded(e)));
        if (eventType !== null) {
  ...
  loginRequest(data) {
    var _this53 = this;
    return _asyncToGenerator(function* () {
      return yield _this53.http.authedRequest(Method.Post, "/login", undefined, data);
    })();
  }
  ...
  logout() {
    ...
    return _asyncToGenerator(function* () {
      ...
      return _this54.http.authedRequest(Method.Post, "/logout");
    })();
  }
  ...
  requestLoginToken(auth) {
    ...
    return _asyncToGenerator(function* () {
      ...
      return _this55.http.authedRequest(Method.Post, "/login/get_token", undefined,
      // no query params
      body, {
  ...
  createRoom(options) {
    var _this56 = this;
    return _asyncToGenerator(function* () {
      var _this56$identityServe;
      // eslint-disable-line camelcase
      // some valid options include: room_alias_name, visibility, invite

      // inject the id_access_token if inviting 3rd party addresses
      var invitesNeedingToken = (options.invite_3pid || []).filter(i => !i.id_access_token);
      if (invitesNeedingToken.length > 0 && (_this56$identityServe = _this56.identityServer) !== null && _this56$identityServe !== void 0 && _this56$identityServe.getAccessToken) {
        var identityAccessToken = yield _this56.identityServer.getAccessToken();
        if (identityAccessToken) {
      ...
      return _this56.http.authedRequest(Method.Post, "/createRoom", undefined, options);
    })();
  }
  ...
  sendStateEvent(roomId, eventType, content) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var room = _this57.getRoom(roomId);
      ...
      yield _this57.encryptStateEventIfNeeded(event, room !== null && room !== void 0 ? room : undefined);
      var pathParams = {
        $roomId: roomId,
        $eventType: event.getWireType(),
      ...
      return _this57.http.authedRequest(Method.Put, path, undefined, event.getWireContent(), opts);
    })();
  }
  encryptStateEventIfNeeded(event, room) {
    var _this58 = this;
    return _asyncToGenerator(function* () {
      if (!_this58.enableEncryptedStateEvents) {
        return;
      }

      // If the room is unknown, we cannot encrypt for it
      if (!room) return;
      if (!_this58.cryptoBackend && _this58.usingExternalCrypto) {
        // The client has opted to allow sending messages to encrypted
        // rooms even if the room is encrypted, and we haven't set up
        // crypto. This is useful for users of matrix-org/pantalaimon
        return;
      }
      if (!_this58.cryptoBackend) {
        throw new Error("This room is configured to use encryption, but your client does not support encryption.");
      }

      // Check regular encryption conditions.
      if (!(yield _this58.shouldEncryptEventForRoom(event, room))) {
        return;
      }

      // If the crypto impl thinks we shouldn't encrypt, then we shouldn't.
      // Safety: we checked the crypto impl exists above.
      if (!(yield _this58.cryptoBackend.isStateEncryptionEnabledInRoom(room.roomId))) {
        return;
      }
      ...
      yield _this58.cryptoBackend.encryptEvent(event, room);
    })();
  }
  ...
  setRoomReadMarkersHttpRequest(roomId, rmEventId, rrEventId, rpEventId) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if ((yield _this59.doesServerSupportUnstableFeature("org.matrix.msc2285.stable")) || (yield _this59.isVersionSupported("v1.4"))) {
        content[ReceiptType.ReadPrivate] = rpEventId;
      }
      return _this59.http.authedRequest(Method.Post, path, undefined, content);
    })();
  }
  ...
  doesServerSupportExtendedProfiles() {
    var _this60 = this;
    return _asyncToGenerator(function* () {
      return (yield _this60.isVersionSupported("v1.16")) || (yield _this60.doesServerSupportUnstableFeature(UNSTABLE_MSC4133_EXTENDED_PROFILES)) || (yield _this60.doesServerSupportUnstableFeature(STABLE_MSC4133_EXTENDED_PROFILES));
    })();
  }
  ...
  getExtendedProfileRequestPrefix() {
    var _this61 = this;
    return _asyncToGenerator(function* () {
      if ((yield _this61.isVersionSupported("v1.16")) || (yield _this61.doesServerSupportUnstableFeature("uk.tcpip.msc4133.stable"))) {
  ...
  getExtendedProfile(userId) {
    var _this62 = this;
    return _asyncToGenerator(function* () {
      if (!(yield _this62.doesServerSupportExtendedProfiles())) {
        throw new Error("Server does not support extended profiles");
      }
      return _this62.http.authedRequest(Method.Get, utils.encodeUri("/profile/$userId", {
        $userId: userId
      }), undefined, undefined, {
        prefix: yield _this62.getExtendedProfileRequestPrefix()
      });
    })();
  }
  ...
  getExtendedProfileProperty(userId, key) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var profile = yield _this63.http.authedRequest(Method.Get, utils.encodeUri("/profile/$userId/$key", {
        $userId: userId,
        $key: key
      }), undefined, undefined, {
  ...
  setExtendedProfileProperty(key, value) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var userId = _this64.getUserId();
      yield _this64.http.authedRequest(Method.Put, utils.encodeUri("/profile/$userId/$key", {
        $userId: userId,
        $key: key
      }), undefined, {
        [key]: value
  ...
  deleteExtendedProfileProperty(key) {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this65.http.authedRequest(Method.Delete, utils.encodeUri("/profile/$userId/$key", {
        $userId: userId,
        $key: key
      }), undefined, undefined, {
  ...
  patchExtendedProfile(profile) {
    ...
    return _asyncToGenerator(function* () {
      ...
      return _this66.http.authedRequest(Method.Patch, utils.encodeUri("/profile/$userId", {
        $userId: userId
      }), {}, profile, {
  ...
  setExtendedProfile(profile) {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this67.http.authedRequest(Method.Put, utils.encodeUri("/profile/$userId", {
        $userId: userId
      }), {}, profile, {
  ...
  addThreePidOnly(data) {
    ...
    return _asyncToGenerator(function* () {
      ...
      return _this68.http.authedRequest(Method.Post, path, undefined, data);
    })();
  }
  ...
  // eslint-disable-next-line camelcase
    ...
    return _asyncToGenerator(function* () {
      ...
      var data = {
        medium,
        address,
        id_server: _this70.getIdentityServerUrl(true)
      };
  ...
  getPushers() {
    var _this71 = this;
    return _asyncToGenerator(function* () {
      var response = yield _this71.http.authedRequest(Method.Get, "/pushers");

      // Migration path for clients that connect to a homeserver that does not support
      // MSC3881 yet, see https://github.com/matrix-org/matrix-spec-proposals/blob/kerry/remote-push-toggle/proposals/3881-remote-push-notification-toggling.md#migration
      if (!(yield _this71.doesServerSupportUnstableFeature("org.matrix.msc3881"))) {
  ...
  identityHashedLookup(addressPairs, identityAccessToken) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var hashes = yield _this72.getIdentityHashDetails(identityAccessToken);
      ...
      var response = yield _this72.http.idServerRequest(Method.Post, "/lookup", params, IdentityPrefix.V2, identityAccessToken);
  ...
  lookupThreePid(medium, address, identityAccessToken) {
    ...
    return _asyncToGenerator(function* () {
      ...
      // convert it manually.
      var response = yield _this73.identityHashedLookup([[address, medium]], identityAccessToken);
  ...
  bulkLookupThreePids(query, identityAccessToken) {
    ...
    return _asyncToGenerator(function* () {
      ...
      // function contract requires a V1 response. We therefore have to
      // convert it manually.
      var response = yield _this74.identityHashedLookup(
  ...
  encryptAndSendToDevice(eventType, devices, payload) {
    var _this75 = this;
    return _asyncToGenerator(function* () {
      if (!_this75.cryptoBackend) {
        throw new Error("Cannot encrypt to device event, your client does not support encryption.");
      }
      var batch = yield _this75.cryptoBackend.encryptToDeviceMessages(eventType, devices, payload);
      ...
      yield _this75.queueToDevice(batch);
    })();
  }
  ...
  getAuthMetadata() {
    ...
    return _asyncToGenerator(function* () {
      var authMetadata;
      try {
        authMetadata = yield _this80.http.request(Method.Get, "/auth_metadata", undefined, undefined, {
        ...
        if (e instanceof MatrixError && e.errcode === "M_UNRECOGNIZED") {
          ...
          var {
            issuer
          } = yield _this80.http.request(Method.Get, "/auth_issuer", undefined, undefined, {
            prefix: ClientPrefix.Unstable + "/org.matrix.msc2965"
          });
